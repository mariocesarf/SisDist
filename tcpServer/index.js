...
server.on('connection', (socket) => {
   // socket.write(chalk.green('Conectado com sucesso'))
    ...
    socket.on('data', (data) => {
        ...
        if(isUserRegistered){
            socket.write(JSON.stringify({error: 'Ja existe um usuario com esse nome!'}))
            socket.end()
        }
        ...
        switch(msg){
            ...
            case '/SAIR':
                socket.write(JSON.stringify({serverMessage: 'Usuario desconectado com sucesso'}))
                socket.end()
            ...
            default:
                ...
                if(req.newNickname){
                    ...
                    storage.broadcast("O usuario\t" + msgOwner + "\t alterou o nickname para\t" + req.newNickname)
                    }
                    else{
                        newSocket.write(JSON.stringify({error: "Ja existe um usuario com esse nickname!"}))
                    }
                    
                }
            
                if(msg) db.forEach( (sock) => {
                    if(msg){
        ...
        /*users.forEach(cliente => {
            let sock = storage.buscarSocket(cliente)
            sock.write(
                JSON.stringify(
