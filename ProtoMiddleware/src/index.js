...
let FeedbackMessageFactory = RequestRepository.lookupType('MiddlewarePackage.FeedbackMessage');
let EditSoundSystemMessageFactory = RequestRepository.lookupType('MiddlewarePackage.EditSoundSystemMessage');

let GateStatusMessageFactory = RequestRepository.lookupType('MiddlewarePackage.GateStatusMessage');
let EditGateStatusMessageFactory = RequestRepository.lookupType('MiddlewarePackage.EditGateStatusMessage');
    ...
    socket.on('data', (data) => {
       // console.log(SensorMessageRequestHandler.decode(data))
        let { Description } = PatternIdentifierMessageRequestHandler.decode(data);
            ...
            console.log(chalk.cyan(`A musica ${ActualTrack} começou a tocar com volume ${Volume}`));
                break;
...
terminal.on('line', (input) => {
    ...
    switch(Command){
        case '/SOM':
            ...
            var EditSoundSystemMessage = EditSoundSystemMessageFactory.encode(EditSoundSystemRequest).finish()
        ...
        case '/ABRIR':
            ...
            var EditGateStatusMessage = EditGateStatusMessageFactory.encode(EditGateStatusMessageRequest).finish()
